---
title: Goldydocs
---

{{< blocks/cover title="Pessebres d'Artesa de Segre" image_anchor="top" height="full" >}}

<a class="btn btn-lg btn-secondary me-3 mb-4" href="https://github.com/google/docsy-example">
Participa al concurs!</i>
</a>
<a class="btn btn-lg btn-warning me-3 mb-4" href="https://github.com/google/docsy-example">
Participa al Repte #Caganer!</i>
</a>
<a class="btn btn-lg btn-primary me-3 mb-4" href="/docs/">
Totes les activitats <i class="fas fa-arrow-alt-circle-right ms-2"></i>
</a>
<p class="lead mt-5 text-white">70é Concurs de Pessebres d’Artesa de Segre i Comarca</p>
{{< blocks/link-down color="info" >}}
{{< /blocks/cover >}}


{{% blocks/lead color="primary" %}}
Els Pessebres del Segre celebren enguany la seva 70a edició! Un concurs obert a tothom: ciutadans, entitats, comerços i
associacions. Tots junts per mantenir viva la tradició del Nadal artesenc i compartir la creativitat de cada racó del
municipi.

{{% /blocks/lead %}}

{{% blocks/section color="secondary" type="row" %}}
<h2 class="text-center pb-4">Com participar?</h2>

{{% blocks/feature icon="fa-1" title="Fes el pessebre!" %}}
Deixa volar la imaginació i construeix el teu pessebre, sol o en grup, amb el material i estil que vulguis.

No et deixis el caganer!
{{% /blocks/feature %}}

{{% blocks/feature icon="fab fa-2" title="Apunta't al concurs!"%}}
Envia'ns el teu pessebre per Whatsapp <a href="https://wa.me/0034625485223" target="_blank">(link)</a> o per correu
concurspessebresartesa@gmail.com

{{% /blocks/feature %}}

{{% blocks/feature icon="fab fa-3" title="Recull el teu premi!" %}}
Vine a l’entrega de premis el 6 de gener i celebra amb nosaltres la tradició dels pessebres d’Artesa!
{{% /blocks/feature %}}


<p class="text-center">Tens dubtes? Llegeix el nostre FAQ o les bases del concurs.</p>
{{% /blocks/section %}}

{{% blocks/section color="primary" %}}

<h2 class="text-center pb-4">Participa al Repte #Caganer!</h2>

<h3 class="pb-4">El pessebre d'Artesa de Segre s'ha perdut!</h3>
Participa al repte del pessebre d’Artesa! Diverses peces del pessebre s’han escampat pels comerços del poble. Busca-les,
escaneja els codis QR i descobreix les pistes que t’ajudaran a resoldre el misteri.
Aconseguiràs trobar qui ha amagat el pessebre?
<div class="pt-4">
A mesura que trobis certs codis, desbloquejaràs fragments del conte del caganer perdut… fins a descobrir qui ha amagat el pessebre!
</div>
<div class="pt-4 text-center">
<a class="btn btn-lg btn-warning me-3 mb-4" href="https://github.com/google/docsy-example">
Participa al Repte #Caganer!</i>
</a>
</div>
{{% /blocks/section %}}

{{% blocks/section color="secondary" %}}

<h2 class="text-center pb-4">Excursió al pessebre del Castellot!</h2>

<div class="row">
<div class="col-lg-4">

 <img src="/castellot.png" alt="AltTetx" style="width:100%; height:auto;">
</div>

<div class="col-lg-8">
Vine a passar un matí especial aquestes festes! 🌟

Farem una caminada fins al Castellot per veure el pessebre i gaudir de les vistes espectaculars d’Artesa.

📅 **Quan?** Dissabte, 27 de desembre, 2025

🕚 **Hora?** Sortida a les 11:00 h

📍 **On?** Davant de l’Església d’Artesa

Hi haurà coca i xocolata per a tothom! 🍫🥖

Una activitat perfecta per compartir natura, tradició i bona companyia. No t’ho perdis, t’hi esperem! 🙌
</div>

</div>
{{% /blocks/section %}}

{{% blocks/section color="primary" %}}
<h2 class="text-center pb-4">Segueix les últimes novetats!</h2>

<behold-widget feed-id="NEY6jHrnJ6FhRoM3OUUW"></behold-widget>
<script>
  (() => {
    const d=document,s=d.createElement("script");s.type="module";
    s.src="https://w.behold.so/widget.js";d.head.append(s);
  })();
</script>
{{% /blocks/section %}}

